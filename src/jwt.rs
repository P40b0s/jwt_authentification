...
impl JWT
{
    ...
    pub fn new_access<I: AsRef<str>, R>(&self, user_id: I, role: R) -> String
    ...
    pub fn new_access_with_audience<I: AsRef<str>, R, A: ToString>(&self, user_id: I, role: R, audience: &[A]) -> String
    where R: for<'de> Deserialize<'de> + Serialize + PartialEq + Clone
    {
        let iat =  OffsetDateTime::now_utc();
        let exp = iat + Duration::minutes(5);
...
mod tests
{
    ...
    async fn gen_key()
    {
        ...
        let key = jwt.new_access(&id, role.to_owned());
        logger::info!("access: {}",  key);
        let upd = jwt.new_access(&id, role.to_owned());
        ...
        let upd2 = jwt.new_access_with_audience(&id, role.to_owned(), &aud);
